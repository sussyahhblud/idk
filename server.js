...
```
if (!username || !jump) {
  return res.status(400).json({ error: 'Missing username or jump name' });
}

if (!WEBHOOK) {
  return res.status(500).json({ error: 'Server not configured (missing webhook)' });
}
...
if (req.file) {
  ...
  if (!discordRes.ok) {
    ...
    if (discordRes.status === 413 || err.includes('too large')) {
      return res.status(413).json({ error: 'File too large for Discord (max 8MB). Please compress it first.' });
    }
  ...
  if (!discordRes.ok) {
    const err = await discordRes.text();
    return res.status(500).json({ error: 'Discord rejected the message: ' + err });
  }
